...
class LoginController extends JControllerLegacy
{
	...
	public function login()
	{
        ...
        //+ 2015.08.24 bejelentkezések, bejelentkezési kisérletek naplózása
		$db = JFactory::getDBO();
		if ($result==1) {
			$db->setQuery('insert into #__tny_naplo 
			(kapcs_id, lastaction, lastact_user_id, lastact_time, lastact_info) 
			select 0, "sikeres LOGIN",u.id,"'.date('Y-m-d H:i:s').'",u.name
			from #__users u
			where u.username = '.$db->quote(JRequest::getVar('username')));
		} else {
			$db->setQuery('insert into #__tny_naplo 
			(kapcs_id, lastaction, lastact_user_id, lastact_time, lastact_info) 
			values 
			(0, "sikertelen LOGIN",0,"'.date('Y-m-d H:i:s').'",'.$db->quote(JRequest::getVar('username')).')');
		}
		$db->query();
		//- 2015.08.24
