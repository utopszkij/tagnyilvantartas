...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JFactory::getApplication()->input->get('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectCimkek_id('".$row->id."', '".$this->escape($row->szoveg)."', '','cimke_id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_tagnyilvantartas&view=cimkek&task=cimkek.edit&id='. $row->cimke_id );
 	$row->id = $row->cimke_id;
 	
	$checked = JHTML::_('grid.id', $i, $row->cimke_id);
