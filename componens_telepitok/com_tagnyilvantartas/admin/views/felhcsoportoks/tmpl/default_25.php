...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JFactory::getApplication()->input->get('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectFelhcsoportok_id('".$row->id."', '".$this->escape($row->nev)."', '','fcsop_id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_tagnyilvantartas&view=felhcsoportok&task=felhcsoportok.edit&id='. $row->fcsop_id );
 	$row->id = $row->fcsop_id;
 	
	$checked = JHTML::_('grid.id', $i, $row->fcsop_id);
