  ...
  if (count($this->items)) : 
  		foreach ($this->items as $i => $item) :
				...
				if ($item->telefon != '') {
					...
					if ($item->telszammegj != '') 
					   $caller = '<a id="tsz'.$item->kapcs_id.'" onclick="telszamClick('.$item->kapcs_id.')" href="tel:'.$s1.'" style="color:red; font-weight:bold">'.
				                 $item->telefon.'</a>';
				    else
					   $caller = '<a id="tsz'.$item->kapcs_id.'" onclick="telszamClick('.$item->kapcs_id.')" href="tel:'.$s1.'">'.$item->telefon.'</a>';
				}
				if ($item->telefon2 != '')
  		...
  		?>
				<tr class="row<?php echo $i % 2; if ($item->ellenorzott==0) echo ' nemellenorzott'; ?>">
					...
					<td><?php echo $item->megjegyzes;?></td>
				</tr>
<?php
