...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_tagnyilvantartas&task=felhasznaloks.saveOrderAjax&tmpl=component';
...
<h2><?php echo JText::_('COM_TAGNYILVANTARTAS_FELHASZNALOK'); ?></h2>
<form action="index.php?option=com_tagnyilvantartas&view=felhasznalok" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
	<table class="adminlist" id="articleList" width="600">
		<thead>
			<tr>
				<th class="px20">
				</th>
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Név', 'a.name', $listDirn, $listOrder ); ?>
				</th>
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Felhasználónév', 'a.username', $listDirn, $listOrder ); ?>
				</th>
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Email', 'a.email', $listDirn, $listOrder ); ?>
				</th>
				<th class="px20">
					<?php echo JHTML::_('grid.sort', 'Azonosító', 'a.id', $listDirn, $listOrder ); ?>
				</th>
							</tr> 			
		</thead>
		<tfoot>
  ...
  if (count($this->items)) : 
  		foreach ($this->items as $i => $item) :
    			...
    			if (JFactory::getApplication()->input->get('function', null)) {
    				$onclick= "onclick=\"window.parent.jSelectFelhasznalok_id('".$item->id."', '".$this->escape($item->szoveg)."', '','user_id')\" ";
    			}  	
...
<input type="hidden" name="task" value="felhasznalok" />
<input type="hidden" name="view" value="felhasznaloks" />
