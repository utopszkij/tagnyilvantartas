...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JFactory::getApplication()->input->get('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectKategoriak_id('".$row->id."', '".$this->escape($row->szoveg)."', '','kategoria_id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_tagnyilvantartas&view=kategoriak&task=kategoriak.edit&id='. $row->kategoria_id );
 	$row->id = $row->kategoria_id;
 	
	$checked = JHTML::_('grid.id', $i, $row->kategoria_id);
  ...
  ?>
	...
	<td>
							...
							<a <?php echo $onclick; ?>href="<?php echo $link; ?>"><?php echo $row->szoveg; ?></a>
								
		</td>	
	
			<td><?php echo $row->kategoria_id; ?></td>		
