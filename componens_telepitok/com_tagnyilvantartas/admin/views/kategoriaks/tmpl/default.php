...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_tagnyilvantartas&task=kategoriaks.saveOrderAjax&tmpl=component';
...
<h2><?php echo JText::_('COM_TAGNYILVANTARTAS_KATEGORIAK'); ?></h2>
<form action="index.php?option=com_tagnyilvantartas&view=kategoriak" method="post" name="adminForm" id="adminForm">
  ...
  if (count($this->items)) : 
  		foreach ($this->items as $i => $item) :
    			...
    			if (JFactory::getApplication()->input->get('function', null)) {
    				$onclick= "onclick=\"window.parent.jSelectKategoriak_id('".$item->id."', '".$this->escape($item->szoveg)."', '','kategoria_id')\" ";
    			}  	
    
 				$link = JRoute::_( 'index.php?option=com_tagnyilvantartas&view=kategoriak&task=kategoriak.edit&id='. $item->kategoria_id );
 	
 				
 	
 				$checked = JHTML::_('grid.id', $i, $item->kategoria_id);
				...
				<tr class="row<?php echo $i % 2; ?>"">
        			<td><?php 
                        ...
                        echo '<input type="radio" id="cb'.$item->kapcs_id.'" name="cid[]"
                           onclick="cbClick(this)" value="'.$item->kategoria_id.'"/>
                        ';
                        ?></td>
					...
					<td class="jobbra"><?php echo $item->kategoria_id; ?></td>
				</tr>
<?php
...
<input type="hidden" name="task" value="kategoriak" />
<input type="hidden" name="view" value="kategoriaks" />
