...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_tagnyilvantartas&task=teruletiszervezeteks.saveOrderAjax&tmpl=component';
...
<h2><?php echo JText::_('COM_TAGNYILVANTARTAS_TERULETISZERVEZETEK'); ?></h2>
<form action="index.php?option=com_tagnyilvantartas&view=teruletiszervezetek" method="post" name="adminForm" id="adminForm">
		<div id="filter-bar" class="btn-toolbar">
			...
			<div class="btn-group pull-left">
                ...
                <button type="button" class="btn hasTooltip" title="<?php echo JText::_('COM_TAGNYILVANTARTAS_TREE_DESC'); ?>" onclick="this.form.filter_search.value='tree';this.form.submit();"><?php echo JText::_('TREE')?></button>
	...
	<table class="adminlist" id="articleList">
		<thead>
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Tulaj_id', 'a.tulaj_id', $listDirn, $listOrder ); ?>
				</th>
				<th class="px20">
  ...
  if (count($this->items)) : 
  		foreach ($this->items as $i => $item) :
    			...
    			if (JFactory::getApplication()->input->get('function', null)) {
    				$onclick= "onclick=\"window.parent.jSelectTeruletiszervezetek_id('".$item->id."', '".$this->escape($item->nev)."', '','terszerv_id')\" ";
    			}  	
    
 				$link = JRoute::_( 'index.php?option=com_tagnyilvantartas&view=teruletiszervezetek&task=teruletiszervezetek.edit&id='. $item->terszerv_id );
 	
 				
 	
 				$checked = JHTML::_('grid.id', $i, $item->terszerv_id);
				...
				<tr class="row<?php echo $i % 2; ?>"">
        			<td><?php 
                        ...
                        echo '<input type="radio" id="cb'.$item->kapcs_id.'" name="cid[]"
                           onclick="cbClick(this)" value="'.$item->terszerv_id.'"/>
                        ';
                        ?></td>
        				
                     <td class="nowrap has-context">
                             <div style="padding-left:<?php echo (20 * $item->level); ?>px">
  								<?php  echo $this->escape($item->nev); ?>
                             </div>   
                    </td>
					<td><?php 
                               $db->setQuery('select * from #__tny_teruletiszervezetek where terszerv_id='.$db->quote($item->tulaj_id));
                               $tulaj = $db->loadObject();
                               echo $tulaj->nev; 
                            ?>
                    </td>
					<td class="jobbra"><?php echo $item->terszerv_id; ?></td>
					</tr>
<?php
