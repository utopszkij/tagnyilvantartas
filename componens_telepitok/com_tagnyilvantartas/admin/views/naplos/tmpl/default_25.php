...
<form action="index.php?option=com_tagnyilvantartas&amp;view=kapcsolatok" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Lastaction', 'a.lastaction', $listDirn, $listOrder ); ?>
				</th>
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Lastact_info', 'a.lastact_info', $listDirn, $listOrder ); ?>
				</th>
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Kapcs_id', 'a.kapcs_id', $listDirn, $listOrder ); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JFactory::getApplication()->input->get('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectKapcsolatok_id('".$row->id."', '".$this->escape($row->lastaction)."', '','kapcs_id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_tagnyilvantartas&view=kapcsolatok&task=kapcsolatok.edit&id='. $row->kapcs_id );
 	$row->id = $row->kapcs_id;
 	
	$checked = JHTML::_('grid.id', $i, $row->kapcs_id);
  ...
  ?>
	...
	<td>
							...
							<a <?php echo $onclick; ?>href="<?php echo $link; ?>"><?php echo $row->lastaction; ?></a>
								
		</td>	
	
			<td><?php echo $row->lastact_info; ?></td>		
	
			<td><?php echo $row->kapcs_id; ?></td>		
