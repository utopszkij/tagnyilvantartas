...
class TagnyilvantartasModelkampanys extends JModelList
{
	public function __construct($config = array())
	{
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array(
                            'megnev', 'a.megnev', 'helyszin', 'a.helyszin',
                            'id', 'a.id',
                        );
        }
	...
	protected function populateState($ordering = null, $direction = null)
	{
			...
			$this->setState('kampanylist.id', $id);			
	...
	protected function getStoreId($id = '')
	{
		...
		$id	.= ':'.$this->getState('kampanylist.id');
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.megnev LIKE ' . $search . ' )');
			}
		}
		...
		if(empty($orderCol)) $orderCol = 'a.megnev';
