...
*/
 defined('_JEXEC') or die('Restricted access');
/**
 * TagnyilvantartasModelKategoriak 
 * @author 
 */
...
class TagnyilvantartasModelKategoriak  extends JModelAdmin { 
...
/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_tagnyilvantartas.kategoriak', 'kategoriak', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_tagnyilvantartas.edit.kategoriak.data', array());
		...
		if(!version_compare(JVERSION,'3','<')){
			$this->preprocessData('com_tagnyilvantartas.kategoriak', $data);
		}
	...
	public function validate(& $form, $data, $group) {
      ...
      $db = JFactory::getDBO();
      $db->setQuery('select * 
                     from #__tny_kategoriak 
                     where szoveg = '.$db->quote($data['szoveg']).' and 
                           kategoria_id<>'.$db->quote($id));
      $res = $db->loadObject();
      if ($res) {
    ...
    */  
    public function delete($pks) {
       ...
       foreach ($pks as $pk) {
           ...
           $db->setQuery('select kapcs_id from #__tny_kapcsolatok where kategoria_id='.$db->quote($pk).' limit 1');
           $res = $db->loadObject();
           if ($res) {
              $errorMsg .= $pk.' '.JText::_('COM_TAGNYILVANTARTAS_CANNOT_DELETE').'<br />';
           }
