...
class TagnyilvantartasModelkategoriaks extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
			...
			$this->setState('kategoriaklist.id', $id);			
	...
	protected function getStoreId($id = '')
	{
		...
		$id	.= ':'.$this->getState('kategoriaklist.id');
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*');
		$query->from('#__tny_kategoriak as a');
 	    $query->join('LEFT OUTER','#__tny_kategoriak k ON k.kategoria_id = a.kategoria_id');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.szoveg LIKE ' . $search . ' )');
			}
		}
		...
		if(empty($orderCol)) $orderCol = 'a.szoveg';
